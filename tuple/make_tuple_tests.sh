...
for i in `seq 1 $2`; do
#  echo "{\"hello$i\",\"world$i\"},"
  echo "tuple<string_view, string_view>{\"hello_hello_hello_hello_$i\",\"world_world_world_world_$i\"},"
done >> $1
}


VS=5000
for i in 1 2 3 4 5 6; do f1 tuple_test$i.cpp ; done
for i in 1 2 ; do f1s tuple_test$i.cpp ; done
f1cs tuple_test3.cpp
f1c tuple_test4.cpp
f1sc tuple_test5.cpp
f1sv tuple_test6.cpp
f3 tuple_test1.cpp $VS
for i in 2 3 ; do f4 tuple_test$i.cpp $VS ; done
for i in 4 5 ; do f5 tuple_test$i.cpp $VS ; done
for i in 6 ; do f6 tuple_test$i.cpp $VS ; done
for i in 1 2 3 4 5 6; do f2 tuple_test$i.cpp ; done

for O in 0 2 3 ; do
  for i in 1 2 3 4 5 6; do
    echo -n tuple_test"$i"_O$O":  "
    echo -n `/usr/bin/time --verbose g++ --std=c++17 tuple_test$i.cpp -O$O -o tuple_test"$i"_O$O 2>&1 | grep "wall clock"`
